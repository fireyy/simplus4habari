			<li id="widget-recentcomments" class="widget widget_comments">
				...
				?>
					<li>
						...
						<?php
							$comments= DB::get_results( 'SELECT * FROM {comments} WHERE post_id = ? AND status = ? AND type = ? ORDER BY date DESC LIMIT 6;', array( $entry->id, Comment::STATUS_APPROVED, Comment::COMMENT ), 'Comment' );
